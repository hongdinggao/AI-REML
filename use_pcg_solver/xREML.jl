...
# based on multi-trait GBLUP model using PCG solver
...
function mt_aireml_gblup(y, ntrait, nind, invGRM, X, Z, G0, R0; tol=1.0e-12)
    ...
    while diff > tol
        ...
        invLHS = LHS \ I
        sol .= cg(LHS, RHS, Pl=Diagonal(LHS), abstol=1e-5)
        e .= y .- W * sol
...
function mt_aireml_gblup_new(y, ntrait, nind, invGRM, X, Z, G0, R0; tol=1.0e-12)
    ...
    while diff > tol 
        ...
        rG = mul_offdiag2(rG)
        Grad = [mat2vec(gG); mat2vec(rG)]
        newlast = size(RHS, 1) + npar
        rhsx = zeros(newlast)
        rhsx[begin:N_EQ] .= RHS
        rhsx[(N_EQ+1):end] .= F' * iRy
        rhsx[(N_EQ+1):end] .= rhsx[(N_EQ+1):end] .- Grad 
        lhss = zeros(newlast, newlast)
        lhss[begin:N_EQ, begin:N_EQ] .= LHS
        lhss[(N_EQ+1):end, (N_EQ+1):end] .= F' * Rinv * F
        lhss[(N_EQ+1):end, begin:N_EQ] .= F' * Rinv * W
        lhss[begin:N_EQ, (N_EQ+1):end] .= transpose(lhss[(N_EQ+1):end, begin:N_EQ])
        sol_new = lhss \ rhsx
        
        thetaAI = [mat2vec(oldG); mat2vec(oldR)] .+ sol_new[(N_EQ+1):end]
        theta = thetaAI
